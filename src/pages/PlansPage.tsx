...
export default function PlansPage({ verticalKey }: { verticalKey: string }) {
  ...
  const classFacetOptions = useAnswersState((state) =>
    state.filters.facets?.flatMap((facet) => facet.options)
  );
      ...
      <div>
        ...
        <AlternativeVerticals
          ...
          customCssClasses={{
            ...
            allCategoriesLink:
              "text-cforange cursor-pointer hover:underline focus:underline",
          }}
        />
        {classFacetOptions &&
          classFacetOptions.length > 0 &&
        ...
        <div className="justify-center space-x-4">
          <VerticalResults
            ...
            customCssClasses={{
              container:
                "sm:flex-col sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:w-4/5 gap-4",
            }}
          />
        </div>
